...
public class SpanTextViewUtil {
    ...
    public static void updateForeColorSpan(TextView tv, int start, int end, String color) {
        try {
            ...
            builder.setSpan(colorSpan, start, Math.min(end, text.length()), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            tv.setText(builder);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static void addForeColorSpan(TextView tv, String text, int tStart, int tEnd, String color) {
        try {
            ...
            spanString.setSpan(span, tStart, Math.min(tEnd, text.length()), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    public static void addClickableSpan(TextView tv, String text, int tStart, int tEnd, final OnClickSpanListener onClickSpanListener) {
        try {
            ...
            builder.setSpan(clickableSpan, tStart, Math.min(tEnd, text.length()), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            tv.setMovementMethod(LinkMovementMethod.getInstance());//不设置，没有点击效果
            tv.append(builder);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static void updateStrikeMySelfSpan(TextView tv, int start, int end) {
        try {
            ...
            SpannableString spanString = new SpannableString(text);
            StrikethroughSpan span = new StrikethroughSpan();
            spanString.setSpan(span, start, Math.min(end, text.length()), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

...
public class ToastUtil {
    private static ArrayList<Toast> mList;
     ...
     * @param yOffset Y轴偏移量
     */
    public static void diyToast(Context context, String msg, int resId, int which, int locat, int yOffset) {
        if (mList == null)
            mList = new ArrayList<>();
        for (Toast toast : mList) {
            toast.cancel();
        ...
        if (locat == Gravity.TOP
                ...
                || locat == Gravity.START
                || locat == Gravity.END) {
            toast.setGravity(locat, 0, yOffset);
        }
        toast.show();
        mList.add(toast);
    }

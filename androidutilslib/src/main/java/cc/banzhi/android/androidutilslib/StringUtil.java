...
import cc.banzhi.android.androidutilslib.regex.RegularExpressionUtil;
...
public class StringUtil {
    ...
    public static String sideTrim(String stream, String trimstr) {
        // null或者空字符串的时候不处理
        if (TextUtils.isEmpty(stream) || TextUtils.isEmpty(trimstr)) {
            return stream;
        }
        // 结束位置
        int epos;
    ...
    public static String formatIdCard(String idCard) {
        if (TextUtils.isEmpty(idCard)
                || !TextUtils.isEmpty(ValidIDCard.iDCardValidate(idCard))) {
            return idCard;
        }

...
public class MediaPlayerUtil {
    ...
    public static synchronized MediaPlayerUtil create(Context context) {
     ...
     * @param flags      种类-特征
     */
    public MediaPlayerUtil setStreamVolume(int streamType, int index, int flags) {
        if (audioManager != null)
     ...
     * @param msec 毫秒数
     */
    public MediaPlayerUtil seekTo(int msec) {
        if (mediaPlayer != null)
    ...
    // 播放1
    public MediaPlayerUtil play(String url) {
    ...
    // 播放2
    public MediaPlayerUtil play(Uri uri) {
        try {
            if (uri != null) {
                // 开启播放
                if (mediaPlayer == null)
